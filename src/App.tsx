...
function StudioPanel() {
  ...
  useEffect(() => {
    ...
    initGoogleLibraries()
      ...
      .catch(err => {
        console.error('Google Libs error:', err);
  ...
  const handleImport = async () => {
    if (!libsReady) {
      ...
      try {
        ...
        setConnectionStatus('error');
        toast.error('Google Libraries Error', {
          description: `Failed to initialize Google libraries: ${err.message || 'Unknown error'}. Please check your connection and API configuration.`,
          duration: 5000,
        });
        return;
      }
    ...
    setConnectionStatus('checking');
    try {
      const picked = await openAssetPicker();
      ...
      toast.promise(
        (async () => {
          for (const item of picked) {
            await addDoc(collection(db, 'assets'), {
              ...
              url: item.url || item.iconUrl || '',
              source: 'Google Drive/Photos',
              authorId: user.uid,
              createdAt: serverTimestamp(),
        ...
        {
          loading: 'Importing assets to your library...',
          success: 'Assets imported successfully!',
          error: (err: any) => `Failed to save assets: ${err.message || 'Unknown error'}`
        }
      );
    } catch (error: any) {
      ...
      const isAuthError = error.message?.includes('Auth failed') || error.message?.includes('GIS Error');
      const isConfigError = error.message?.includes('missing') || error.message?.includes('no encontrada');
      
      toast.error('Google Connection Error', {
        description: isAuthError 
          ? "Authentication failed. Make sure you've granted the necessary permissions in the Google popup."
          : isConfigError
          ? "Missing Google API configuration. Please check your Client ID and API Key in the environment settings."
          : `An error occurred: ${error.message || 'Check your API keys and connection.'}`,
        duration: 8000,
        action: {
          label: 'Retry',
  ...
  const handleEdit = async (asset: any) => {
    try {
      await createDesignWithMedia(asset.url);
    } catch (error) {
  ...
  const handleLocalUpload = async (file: File) => {
    ...
    try {
      const result = await uploadAsset(user.uid, file);
      await addDoc(collection(db, 'assets'), {
        name: result.name,
        url: result.url,
        source: 'local',
  ...
  const filteredAssets = useMemo(() => {
    ...
    return assets.filter(a =>
      sourceFilter === 'local' ? a.source === 'local' : a.source !== 'local'
    );
  }, [assets, sourceFilter]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div className="text-center md:text-left">
          <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-accent mb-4 font-mono">VISUAL_STUDIO_V1.1</div>
          <h2 className="font-display text-5xl font-semibold mb-4 leading-tight tracking-tight">Content Studio</h2>
          <p className="text-sm text-ink-muted max-w-xl font-medium leading-relaxed font-sans mx-auto md:mx-0">
            Professional design pipeline: Import from Google Drive, Photos or upload directly from your computer.
          </p>
        </div>
        <div className="flex flex-col items-center md:items-end gap-2">
          <button
            ...
            className="px-8 py-4 bg-accent text-white rounded-2xl font-bold text-xs tracking-widest uppercase hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-accent/20 flex items-center justify-center gap-3"
          >
            {isImporting ? <RefreshCcw className="animate-spin" size={16} /> : <HardDrive size={16} />}
            Import from Drive
          </button>
          ...
          {connectionStatus !== 'idle' && (
            <div className={cn(
              ...
              connectionStatus === 'checking' ? "text-amber-custom bg-amber-light border-amber-custom/20" :
              connectionStatus === 'connected' ? "text-green-custom bg-green-light border-green-custom/20" :
              "text-rose-500 bg-rose-50 border-rose-100"
            )}>
              {connectionStatus === 'checking' ? 'INITIALIZING_DRIVE...' :
               connectionStatus === 'connected' ? 'GOOGLE_DRIVE_READY' :
               'CONNECTION_ERROR'}
            </div>
          )}
        </div>
      ...
      <div className="mb-8 space-y-4">
        <DropZone onFileUpload={handleLocalUpload} disabled={isUploadingLocal} />
        {isUploadingLocal && (
        ...
        <div className="flex gap-2">
          {(['all', 'local', 'drive'] as const).map(filter => (
            <button
              key={filter}
            ...
            >
              {filter === 'all' ? 'All' : filter === 'local' ? 'Local' : 'Cloud'}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filteredAssets.map((asset) => (
          <div key={asset.id} className="bg-card border border-brd p-4 rounded-custom hover:shadow-custom transition-all group relative overflow-hidden flex flex-col h-full">
            <div className="aspect-square bg-paper rounded-2xl mb-4 overflow-hidden relative flex items-center justify-center border border-brd">
              {asset.url ? (
                ...
                <ImageIcon size={40} className="text-ink-muted opacity-20" />
              )}
            ...
            <div className="flex-1">
              <h4 className="text-xs font-bold text-ink truncate mb-1">{asset.name}</h4>
              <p className="text-[9px] text-ink-muted font-mono uppercase tracking-widest">{asset.source}</p>
            </div>
            <div className="flex justify-end mt-4 pt-4 border-t border-brd/50">
              <button 
        ...
        {assets.length === 0 && !isImporting && (
          <div className="col-span-full py-32 border-2 border-dashed border-brd rounded-custom flex flex-col items-center justify-center text-ink-muted gap-4 opacity-40">
             <LayoutGrid size={48} />
             <p className="font-mono text-[10px] font-bold uppercase tracking-widest">No assets imported yet</p>
          </div>
        )}
      </div>
    </div>
